...
@section('content')
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button"
                                        class="text-amber-600 hover:text-amber-800 cursor-pointer delete-user-btn"
                                        title="Edit" data-user-id="{{ $user->id }}"
                                        data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}"
                                        data-user-username="{{ $user->username }}" data-user-email="{{ $user->email }}"
                                        data-user-photo="{{ $user->photo }}" data-modal-target="updateModal"
            ...
            <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
                ...
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST" id="updateUserForm"
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <button type="button" data-modal-toggle="updateModal"
                            class="text-gray-700 inline-flex items-center bg-white hover:bg-gray-100 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                            Cancel
                        </button>
                    </div>
    ...
    <script>
        ...
        function updatePreviewName(name) {
            ...
            // Only update if no custom photo is uploaded
            if (!photoInput.files || photoInput.files.length === 0) {
                const encodedName = encodeURIComponent(name || 'User');
                preview.src = `https://ui-avatars.com/api/?name=${encodedName}&background=FFAB2F&color=fff&size=128`;
            }
        }
        ...
        // Update preview avatar when name changes (for Update Modal)
        function updatePreviewNameUpdate(name) {
            const preview = document.getElementById('updatePhotoPreview');
            const photoInput = document.getElementById('update-photo');

            // Only update if no custom photo is uploaded
            if (!photoInput.files || photoInput.files.length === 0) {
                // Check if current photo is from UI Avatars
                if (preview.src.includes('ui-avatars.com')) {
                    const encodedName = encodeURIComponent(name || 'User');
                    preview.src = `https://ui-avatars.com/api/?name=${encodedName}&background=FFAB2F&color=fff&size=128`;
                }
            }
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (userPhoto) {
                        ...
                        photoPreview.src =
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=FFAB2F&color=fff&size=128`;
                    }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Set form action dengan route destroy (sesuaikan dengan route Anda)
                    deleteForm.action = `/admin/users/${postId}`;

                    // Set post title di modal
